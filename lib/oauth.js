 ...
 * @param user
 * @param config
 * @return {{registerProvider: registerProvider, registerOAuth2: registerOAuth2, registerTokenProvider: registerTokenProvider}}
 */
module.exports = function(router, passport, user, config) {
  ...
  // Function to publish a session|error|link following authentication from an OAuth provider
   ...
   * @param {(credentials:object|undefined, passport:passport.PassportStatic, authHandler:{req:any, provider:string, auth:{access_token?:string, refresh_token?:string}, profile:{}})=>void} configFunction
   */
  function registerProvider(provider, configFunction) {
    ...
    if (config.getItem(configRef + '.credentials')) {
      ...
      router.get('/' + provider, passportCallback(provider, options, 'login', false), oauthErrorHandler);
      router.get('/' + provider + '/callback', passportCallback(provider, options, 'login', true), initSession, oauthErrorHandler);
      router.get('/' + provider + '/publish', publishOAuthEvent, generalAuthErrorHandler);
      if(!config.getItem('security.disableLinkAccounts')) {
        router.get('/link/' + provider, passport.authenticate('bearer', {session: false}), passportCallback(provider, options, 'link', false), oauthErrorHandler);
        router.get('/link/' + provider + '/callback', passport.authenticate('bearer', {session: false}),
          passportCallback(provider, options, 'link', true), linkSuccess, oauthErrorHandler);
      }
  ...
  function isValidCustomCallbackUri(uri)
  {
    ...
    // accept cdx skin login redirect uri like "https://cloud.cleverdox.de/app/main/adl/12313/login/de.cleverdox.demo"
    return /^https:\/\/[^/]+\/app\/main\/adl\/[a-z0-9]+\/login\/([a-z]{2,}\.[a-z]{2,}\.[a-z]{2,}[a-z.]*)|false$/.test(uri);
   ...
   */
  function passportCallback(provider, options, operation, callback) {
    return function(req, res, next) {
      ...
      if(!callback)
      {
          // initial request to oAuth provider
          ...
          if(operation === 'login'){
            ...
            if(ec) {
              // feature for client side event listener for server side login event
              if(!_channels[ec])
              ...
              {
                // cleanup session on initial call
  ...
  function getPublishRedirectURL(provider, req, event)
  {
    ...
    return reqUrl + 'token=' + encodeURIComponent(JSON.stringify(buildTokenData(event.error, event.auth, event.link, jsCallback)));
  }
   ...
   * @param {string} auth
   * @param {string} link
   * @param {boolean} jsCallback
   * @returns {{error?:string, auth?:string, link?:string, jsCallback?:boolean}}
   */
  function buildTokenData(error, auth, link, jsCallback){
    const token = {};
    if(error)
      token.error = error;
    if(auth)
    ...
    if(link)
      token.link = link;
    if(jsCallback === true || jsCallback === false)
      token.jsCallback = jsCallback;
    return token;
  }
  ...
  function getProvider(pathname) {
    ...
    if(items.length === 2 && items[0].length ===0)
      return assertNotEmpty(items[1], "cannot extract provider from " + pathname + " at index 1"); // "/<provider>"
    let index = items.indexOf('callback');
    if(index > 0) {
      return assertNotEmpty(items[index-1], "cannot extract provider from " + pathname + " at index " + index);
    }
    index = items.indexOf('publish');
    if(index > 0) {
      return assertNotEmpty(items[index-1], "cannot extract provider from " + pathname + " at index " + index);
    }
    throw new Error("cannot extract provider from " + pathname);
  }
   ...
   * @returns {string}
   */
  function assertNotEmpty(v, message){
    if(!v || v.length ===0)
