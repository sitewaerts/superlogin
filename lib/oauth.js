...
module.exports = function(router, passport, user, config) {
    ...
    registerProvider(providerName, function (credentials, passport, authHandler) {
      passport.use(providerName, new Strategy(credentials,
        ...
        if(!profile.username)
        {
          if(params.id_token)
          {
              // TODO: move to special passport-azure-ad-oauth2 derived provider impl
              ...
              const userInfo = jsonwebtoken.decode(params.id_token);
              if(userInfo)
              {
                ...
                profile.username = profile.upn || profile.unique_name || profile.email;
              }
              profile = profile || {};
          }
        }
  ...
  function isValidCustomCallbackUri(uri)
  {
    ...
    // accept only schemes that look like "de.cleverdox.demo[.xx.xx.xx]://"
    return /^[a-z]{2,}\.[a-z]{2,}\.[a-z]{2,}[a-z.]*:\/\/$/.test(uri);
  ...
  function getLinkCallbackURLs(provider, req, operation, accessToken) {
    ...
    //TODO: check req.session.customrRedirectUrl
