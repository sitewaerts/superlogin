...
module.exports = function(router, passport, user, config) {
  ...
  function registerProvider(provider, configFunction) {
    ...
    if (config.getItem(configRef + '.credentials')) {
      ...
      router.get('/' + provider, passportCallbackPreparation, passportCallback(provider, options, 'login'), oauthErrorHandler);
  ...
  function passportCallbackPreparation(req, res, next) {
      if(req.session && req.session.isNew)
      {
        // first access, we need an additional redirect to propagate the session cookie to the browser
