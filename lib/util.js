...
{
    return URLSafeBase64.encode(uuid(null, Buffer.alloc(16)));
};
...
util.addProvidersToDesignDoc = function (config, ddoc)
{
    ...
    const ddocTemplate =
        ...
        "    emit(doc['%PROVIDER%'].profile.id.toLowerCase(), null);\n" +
        "  }\n" +
        "}";
 ...
 * @param {Array<V>} b Second array
 * @return {Array<V>} resulting array
 */
