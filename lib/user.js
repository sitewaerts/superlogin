...
module.exports = function User(config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.isEmail = function(username){
    return username.match(EMAIL_REGEXP);
  }
  ...
  this.get = function (login) {
    if(!login || login.length === 0)
      return Promise.reject("missing login");
    var query;
    if(emailUsername) {
      query = 'emailUsername';
    } else {
   ...
   * @param {any} profile
   * @param {any} req
   * @return {Promise<any>}user object for newly created or existing user
   */
  this.socialAuth = async function(provider, auth, profile, req) {
     ...
     */
    const trustedProviders = config.getItem(configRef + ".trustedProviders")
    ...
    if(!profile.id)
      throw new Error("missing profile.id in " + JSON.stringify(profile));
    ...
    try {
       ...
       * @type {Array<string>}
       */
      const defaultRoles = util.trimStringArray([].concat(config.getItem('security.defaultRoles'), config.getItem(configRef + ".defaultRoles")));

      const results = await userDB.query('auth/' + provider, {key: profileId, include_docs: true});

      if (results.rows.length > 0) {
        // username already found for given provider
        user = results.rows[0].doc;

        // update default roles
        user.roles = util.arrayUnion(user.roles, defaultRoles);
      }
      else {
        // username not yet found for given provider
        ...
        if (trustedProviders) {
          // check if user already registered via any of the trusted providers
          for (let trustedProvider of trustedProviders) {
            if(trustedProvider === provider)
              continue;
            const trusted = await userDB.query('auth/' + trustedProvider, {
              key: profileId,
              include_docs: true
            });
            if (trusted.rows.length > 0) {
              ...
              user.providers = user.providers.concat(provider);
              if (profile.emails?.length > 0 && !user.email) {
                user.email = profile.emails[0].value;
              }

              // TODO: if(emailUsername) ... else ...
        ...
        if (!user) {
          ...
          if (emailUsername) {
            ...
            try {
              await self.validateEmailUsername(user.email);
            ...
            if (profile.username) {
              ...
              if (user.email) {
                ...
                baseUsername = profileId;
              }
            }
            try {
              await self.validateEmailUsername(user.email);
              user._id = await generateUsername(baseUsername, errorOnDuplicate)
            } catch (e) {
              emailFail(e);
            }
          }
      ...
      const userDoc = newAccount ? await addUserDBs(user) : user;
      const action = newAccount ? 'signup' : 'login';

      await self.logActivity(userDoc._id, action, provider, req, userDoc);

      const finalUser = newAccount ?
          await processTransformations(onCreateActions, userDoc, provider) :
      ...
      .then(function() {
        ...
        if(typeof user.personalDBs === 'object') {
          ...
          if(config.getItem('dbServer.publicURL')) {
            const dbObj = new URL(config.getItem('dbServer.publicURL'));
            dbObj.auth = newSession.token + ':' + newSession.password;
            publicURL = dbObj.toString();
          } else {
   ...
   * @param {string} base
   * @param {boolean} [errorOnDuplicate]
