...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.refreshSession = function (key) {
    return this.updateSession(key, true);
  };

  this.syncSessionRoles = function (key) {
    return this.updateSession(key, false);
  };

  this.updateSession = function (key, refresh) {
    var newSession;
    return session.fetchToken(key)
      ...
      .then(function(finalUser) {
        ...
        .then(function() {
          // TODO: pass new roles, expires, etc to dbAuth
          // expires is not needed there because dbAuth grants access as long as the key is not deleted
          // but roles are needed there ....
          //return dbAuth.updateKey(newSession._id, key, newSession.expires, newSession.refreshed, newSession.roles);
        })
      .then(function() {
