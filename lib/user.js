...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.getAllRolesFromUserDoc = function(userDoc){
    const roles = [].concat(userDoc.roles);
  ...
  const userModel = {
    ...
    static: {
      type: 'user',
      roles : [].concat(config.getItem('security.defaultRoles'), config.getItem("local.defaultRoles")),
      providers: ['local']
    },
    rename: {
      username: '_id'
    ...
    return BPromise.resolve()
      ...
      .then(function(results) {
        if (results.rows.length > 0) {
          ...
          user.type = 'user';
          user.roles = [].concat(config.getItem('security.defaultRoles'), config.getItem(configRef + ".defaultRoles"));
          user.signUp = {
            provider: provider,
            timestamp: new Date().toISOString(),
            ip: ip
          ...
          if(emailUsername) {
            if(!user.email){
              if(profile.username && email.match(profile.username))
  ...
  this.updateSession = function (key, refresh, cleanup) {
    ...
    return session.fetchToken(key)
      .then(function(oldToken) {
        ...
        if(!newSession)
            return refresh ? BPromise.reject('no session for key "' + key + '"') : null;

        if(isSessionExpired(newSession))
        {
          return session.deleteTokens([key]).then(()=>{
            return BPromise.reject('session for key "' + key + ' already expired"');
          })
        }
  ...
  function generateUsername(base, errorOnDuplicate) {
    ...
    return userDB.allDocs({startkey: base, endkey: base + '\uffff', include_docs: false})
      .then(function(results){
        ...
        if(errorOnDuplicate)
          return BPromise.reject('account name already exists');
        let num = 0;
        while(!finalName) {
          num++;
