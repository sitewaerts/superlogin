...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.validateUsername = function (username) {
    ...
    if (!username.match(USER_REGEXP)) {
      return BPromise.resolve('invalid');
    }
      ...
      .on('change', function (change)
      {
          if (change.deleted)
          {
              userDB.get(change.id, {revs: true, open_revs: 'all'})
                  .then(function (infos)
                  {
                      console.log('deleted', {change, infos});
                      ...
                      return userDB.get(change.id, {rev: lastRev})
                          .then(function (user)
                          {
                              function _deleteDBS()
                              {
                                  ...
                                  {
                                      return BPromise.all(Object.keys(user.personalDBs).map(function (db)
                                      {
          ...
          else
          {
            ...
            return self.cleanupSessions(userDoc)
                .then(function(finalUser){
                    ...
                    sequential(Object.keys(userDoc.session).map(function (key)
                    {
                      return function()
                      {
                        return self.syncSessionRoles(key)
                            ...
                            {
                              console.error('cannot sync session roles (' + key + ')', error, userDoc);
                            });
                      }
                    })).catch(function (error)
                    {
                      console.error('cannot sync session roles', error);
                    });
                });
          }
      });

  this.cleanupSessions = function(userDoc, op){
    var oldSessions = Object.keys(userDoc.session = userDoc.session || []);
    op = op || 'expired';
    return self.logoutUserSessions(userDoc, op)
        .then(function(finalUser){
          if(!util.arrayEquals(oldSessions, Object.keys(finalUser.session)))
            return userDB.put(finalUser);
          return finalUser;
        });
  }
  ...
  this.updateSession = function (key, refresh, cleanup) {
    ...
    return session.fetchToken(key)
      .then(function(oldToken) {
        ...
        {
          if(refresh)
            return BPromise.reject('no session for key "' + key + '"');
           return newSession;
        }
        ...
        return userDB.get(newSession._id)
            .then(function(userDoc){
              if(!util.arrayEquals(newSession.roles, userDoc.roles))
              {
                changed = true;
                rolesChanged = true;
                newSession.roles = userDoc.roles;
              }
              if(changed)
            ...
            .then(function() {
              if(rolesChanged)
              {
                ...
                if (user.personalDBs && user.session)
                  return dbAuth.authorizeUserSessions(user._id, user.personalDBs, [key], user.roles);
              }
            })
