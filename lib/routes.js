...
module.exports = function (config, router, passport, superlogin, user)
{
    ...
    function getResponseType(req){
        return req.query['response_type'] || req.body['response_type'] || 'token';
    }

    function isOAuth(req){
        return getResponseType(req) === 'code';
    }
    ...
    function prepareUrlForRedirect(reqUrl){
        if (reqUrl.indexOf('#') > 0)
    ...
    function buildTokenRedirect(req, auth, error)
    {
        ...
        if (!reqUrl || !token)
            return null;
        return prepareUrlForRedirect(reqUrl) + 'token=' + encodeURIComponent(JSON.stringify(token));
    }

    function buildCodeRedirect(req, code)
    {
        let reqUrl = req.body['redirect_uri_success'] || req.body['redirect_uri'];
        if (!reqUrl)
            return null;
        return prepareUrlForRedirect(reqUrl) + 'code=' + encodeURIComponent(code);
    }
    ...
    router.post('/login', function (req, res, next)
    {
        ...
        {
            const oAuthRequest = isOAuth(req);
            if (err)
            {
                console.error("cannot passport.authenticate", err);
                const redirect = buildTokenRedirect(req, null, err);
                if (redirect)
            ...
            if (!user)
            {
                ...
                const redirect = buildTokenRedirect(req, null, {message: 'login 401', info: info});
                if (redirect)
                {
            ...
            if(oAuthRequest)
            {
                ...
                oAuthCodes[code] = user;

                return res.status(302).redirect(buildCodeRedirect(req, code));
            }
    ...
    }, function (req, res, next)
    {
        ...
        return superlogin.createSession(req.user._id, 'local', req)
            .then(function (mySession)
            {
                ...
                const redirect = buildTokenRedirect(req, auth, null);
                if (redirect)
                {
    ...
    router.get('/oauth/profile', function (req, res, next){
        ...
        if(!result.email && superlogin.isEmail(user.username))
