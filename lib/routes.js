        ...
        return superlogin.createSession(req.user._id, 'local', req)
            .then(function (mySession)
            {
                ...
                if(req.body['redirect_uri'])
                {
                    ...
                    const auth =  {token: mySession.token, password: mySession.password};
                    if(reqUrl.indexOf('?') >=0)
                        reqUrl = reqUrl + "&";
                    else
                        reqUrl = reqUrl + "?";

                    reqUrl = reqUrl + 'token=' + encodeURIComponent(JSON.stringify({auth: auth}));
                    res.status(302).redirect(reqUrl);
                }
                else {
