...
module.exports = function (config, router, passport, superlogin, user)
{
    ...
    }, function (req, res, next)
    {
        ...
        return superlogin.createSession(req.user._id, 'local', req)
            ...
            {
                const redirect = buildRedirect(req, mySession, null);
                if(redirect)
                {
                    res.cookie('localAuth', JSON.stringify(mySession), {expires: new Date(mySession.expires), secure: true, path: getCookiePath(req)})
